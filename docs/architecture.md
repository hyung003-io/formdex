# formdex architecture

## what it does

takes a PDF form URL and autonomously produces a trained YOLOv8 model for form field detection through an iterative loop: generate synthetic data, augment, train, evaluate, repeat until target accuracy.
...
│   ├── collect/             # download PDF, generate synthetic form images
...
├── formdex.sh               # ralph-style autonomous loop orchestrator
...
autonomous: populate config.json, then `bash formdex.sh`
