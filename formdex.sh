...
MAX_ITERATIONS="${1:-10}"
LOG_FILE="$SCRIPT_DIR/formdex.log"
...
[ ! -f "$SCRIPT_DIR/progress.txt" ] && echo "# FormDex Progress Log" > "$SCRIPT_DIR/progress.txt"

for i in $(seq 1 $MAX_ITERATIONS); do
  echo "=== FormDex Iteration $i/$MAX_ITERATIONS ==="

  OUTPUT=$(codex exec --full-auto \
    "Execute the next phase of the FormDex pipeline. Read AGENTS.md for iteration logic.
