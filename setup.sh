#!/usr/bin/env bash
# FormDex setup script — installs all dependencies and verifies the environment
set -euo pipefail
...
echo -e "${BLUE}
 ...
 ██       ██████  ██   ██ ██      ██ ██████  ███████ ██   ██
${NC}"
echo -e "${BLUE}Intelligent PDF form field detection & extraction${NC}"
echo ""
...
if check_cmd codex; then
  ...
  warn "codex CLI not found (optional — needed for parallel labeling and formdex.sh)"
...
echo "     bash formdex.sh          # autonomous loop"
